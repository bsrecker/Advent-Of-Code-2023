...
# Initialize the color_counts dictionary
color_counts = {}

actual_bag_data = {'blue': 14, 'green': 13, 'red': 12}
...
for i, line in enumerate(file):
    ...
    # Check if the game_id is already in color_counts, if not, initialize it
    if game_id not in color_counts:
        color_counts[game_id] = {}
    ...
    for record in list_of_game_records:
        for color in record:
            ...
            if actual_bag_data[color_name] >= count:
                possible_outcomes += 1

            if possible_outcomes == sum(map(len, list_of_game_records)):
                sum_of_ids += i + 1
                print(game_id)
